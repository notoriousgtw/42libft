...
/*   Updated: 2018/08/10 14:09:17 by gwood            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		    *ft_sha256(t_byte *message, size_t length)
{
	...
	while (i < d->length / 64)
        chunk_loop(d, i++);
    digest = ft_strnew(64);
	digest = ft_strjoinfree(digest, ft_uitoa(d->h0, 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(d->h1, 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(d->h2, 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(d->h3, 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(d->h4, 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(d->h5, 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(d->h6, 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(d->h7, 16), 2);
    digest[64] = 0;
    free(d->msg);
    free(d);
	return (digest);
