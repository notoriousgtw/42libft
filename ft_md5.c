...
/*   Updated: 2018/08/10 13:15:53 by gwood            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char		    *ft_md5(t_byte *message, size_t length)
{
	...
	while (d->offset < d->length)
        word_loop(d);
    digest = ft_strnew(32);
	digest = ft_strjoinfree(digest, ft_uitoa(ft_bswap32(d->h0), 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(ft_bswap32(d->h1), 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(ft_bswap32(d->h2), 16), 2);
	digest = ft_strjoinfree(digest, ft_uitoa(ft_bswap32(d->h3), 16), 2);
    digest[32] = 0;
    free(d);
	return (digest);
}
